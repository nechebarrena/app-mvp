...
from .tasks import (
    ...
    get_server_model_config, set_server_model_config
)
...
async def set_tracking_backend_endpoint(data: dict):
...
async def get_models_config():
    ...
    return get_server_model_config()


@app.post("/api/v1/config/models", tags=["config"])
async def set_models_config(data: dict):
    """Set server-side model configuration (tracking backend + optional models)."""
    set_server_model_config(data)
    return {"success": True, **get_server_model_config()}
